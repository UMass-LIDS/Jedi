...
class node:
    ...
    def __init__(self, obj_id, size):
        ...
        self.is_root = False
        self.last_access = -1
        self.min_residual_popularity = 0
        self.max_residual_popularity = 0
        node.counter += 1
    ...
    def initResidualPopularity(self, p):
        self.min_residual_popularity = p
        self.max_residual_popularity = p
    ...
    def if_any_ones(self, checked_until, check_till):
        if checked_until >= check_till:
            return None, checked_until

        if len(self.children) == 0:
            if self.max_residual_popularity == 1:
                checked_until += 1
                return self, checked_until
            else:
                checked_until += 1
                return None, checked_until

        for c in self.children:
            k, checked_until = c.if_any_ones(checked_until, check_till)
            if k != None:
                break

        return k, checked_until

    
    def find_top_objects_in_stack(self, top_objects_in_stack, limit):
        if len(top_objects_in_stack) >= limit:
            return

        if len(self.children) == 0:
            top_objects_in_stack.append([self, self.max_residual_popularity])
            return

        for c in self.children:
            if c.max_residual_popularity > 1:
                c.find_top_objects_in_stack(top_objects_in_stack, limit)
